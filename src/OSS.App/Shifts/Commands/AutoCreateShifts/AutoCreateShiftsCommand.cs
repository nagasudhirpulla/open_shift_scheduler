...
public class AutoCreateShiftsCommand : IRequest<bool>
{
    ...
    public class AutoCreateShiftsCommandHandler : IRequestHandler<AutoCreateShiftsCommand, bool>
    {
        ...
        public async Task<bool> Handle(AutoCreateShiftsCommand request, CancellationToken cancellationToken)
        {
            ...
            List<int> orderedShiftTypeIds = await _context.ShiftCycleItems.OrderBy(sci => sci.ShiftSequence).Select(sci => sci.ShiftTypeId).ToListAsync(cancellationToken: cancellationToken);
            ...
            List<Shift> startDayShifts = await _context.Shifts.Include(s => s.ShiftType).Include(s => s.ShiftParticipations).Where(s => s.ShiftDate == startDate).ToListAsync(cancellationToken: cancellationToken);
            ...
            foreach (Shift startDayShift in startDayShifts)
            {
                ...
                for (DateTime shiftDate = startDate.AddDays(1); (shiftDate.Date <= endDate.Date) && (currentShiftTypeIndex != -1); shiftDate = shiftDate.AddDays(1))
                {
                    ...
                    List<Shift> existingShifts = await _context.Shifts.Where(s => s.ShiftDate == shiftDate && s.ShiftTypeId == currentShiftTypeId).ToListAsync(cancellationToken: cancellationToken);
