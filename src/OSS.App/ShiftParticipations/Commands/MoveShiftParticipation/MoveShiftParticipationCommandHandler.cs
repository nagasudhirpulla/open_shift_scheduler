...
public class MoveShiftParticipationCommandHandler : IRequestHandler<MoveShiftParticipationCommand, Shift>
{
    ...
    public async Task<Shift> Handle(MoveShiftParticipationCommand request, CancellationToken cancellationToken)
    {
        ...
        ShiftParticipation shiftParticipation = await _context.ShiftParticipations.FindAsync(new object[] { request.ShiftParticipationId }, cancellationToken: cancellationToken);
        ...
        Shift shift = await _context.Shifts.Include(s => s.ShiftParticipations).FirstOrDefaultAsync(s => s.Id == shiftParticipation.ShiftId, cancellationToken: cancellationToken);
        if (shift == null)
        {
            return null;
        }
        ...
        return await _context.Shifts.Include(s => s.ShiftParticipations).FirstOrDefaultAsync(s => s.Id == shift.Id, cancellationToken: cancellationToken);
    }
}
